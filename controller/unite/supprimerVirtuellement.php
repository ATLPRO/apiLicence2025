...
if (!isset($donnees['idU'])) {
    http_response_code(400);
    echo json_encode(["success" => false, "message" => "Code de l'unite manquant."]);
    exit;
}
...
try {
    ...
    $resultat = $unite->supprimerVirtuellement($donnees['idU']);

    if ($resultat) {
        echo json_encode(["success" => true, "message" => "Unite supprimé  avec succès."]);
    } else {
