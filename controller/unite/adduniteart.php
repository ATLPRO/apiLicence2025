...
// Créer une instance de la classe famille
    $unite = new uniteart($db);
    $data = json_decode(file_get_contents("php://input"));

     // Remplir les propriétés de la famille
    $unite->intituleU = htmlspecialchars($data->intituleU);
    $unite->QteU = htmlspecialchars($data->QteU);
    $unite->PuU = htmlspecialchars($data->PuU);
     

    $result=$unite->createuniteart();
    ...
    if ($result) {
        ...
        echo json_encode(['message' => "unite enregistrée avec succès"]);
    } else {
        // Une opération a échoué, on annule la transaction
        $db->rollBack();
        http_response_code(503);
        echo json_encode(['message' => "Échec de l'enregistrement de l'unite"]);
    }
} 
 else {
