...
if (isset($_GET['host']) && isset($_GET['dbname']) && isset($_GET['username']) && isset($_GET['password'])) {
    ...
    // Récupération de la liste des familles avec leurs détails
    $listefa = $famille->readAll();
    if (!empty($listefa)) {
        // Renvoyer la liste des familles au format JSON
        http_response_code(200);
        echo json_encode($listefa);
    } else {
        http_response_code(404);
        echo json_encode(["message" => "Aucune famille trouvé "]);
    }
} else {
