...
if (!isset($donnees['idArt'])) {
    ...
    echo json_encode(["success" => false, "message" => "Code de l'article manquant."]);
    exit;
}
    // Créer une instance de la classe Database avec les paramètres de connexion fournis
    $database = new database($host, $dbname, $username, $password);
    $db = $database->getConnexion();
    // Créer une instance de la classe nomenclature
    $com = new nomenclature($db);
    $com->idArt = $donnees['idArt'];
    // Récupération de la liste des matieres premiers 
    $listedoc = $com->getComposantsByIdArt($donnees['idArt']);
    if (!empty($listedoc)) {
        ...
        echo json_encode(["message" => "Aucune matiere premiere trouvée "]);
    }
} else {
    http_response_code(400);
