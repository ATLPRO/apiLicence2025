...
if (!isset($donnees['refArt'])) {
    http_response_code(400);
    echo json_encode(["success" => false, "message" => "reference article manquant."]);
    exit;
}
    ...
    $com = new article($db);
    $com->refArt = $donnees['refArt'];
    // Récupération de la liste des commande avec leurs détails
    $listedoc = $com->refDesTypeArt($donnees['refArt']);
    if (!empty($listedoc)) {
