...
if (isset($_GET['host']) && isset($_GET['dbname']) && isset($_GET['username']) && isset($_GET['password'])) {
    ...
    // Récupération de la liste des personnel avec leurs détails
    $listeart = $personnel->getAll();
    if (!empty($listeart)) {
        // Renvoyer la liste des personnel au format JSON
        http_response_code(200);
        echo json_encode($listeart);
    } else {
        http_response_code(404);
        echo json_encode(["message" => "Aucun personnel trouvé "]);
    }
} else {
