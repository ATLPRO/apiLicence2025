...
if (!isset($donnees['matriculePers'])) {
    http_response_code(400);
    echo json_encode(["success" => false, "message" => "Code du personnel manquant."]);
    exit;
}
...
try {
    ...
    $resultat = $personnel->supprimerVirtuellement($donnees['matriculePers']);

    if ($resultat) {
        echo json_encode(["success" => true, "message" => "personnel supprimé  avec succès."]);
    } else {
